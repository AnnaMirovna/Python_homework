...
def get_info():
    ...
    # first_name = 'Ivan'
    # last_name = 'Ivanov'
    info.append(input("Input last name: "))
    info.append(input("Input first name: "))
    flag = False
    while not flag:
        try:
    ...
    while True:
        ...
        if command == 'q':
            ...
            if not exists('phone.csv'):
                ...
                edit_file(input("Что изменить: "))
        elif command == 'd':
            if not exists('phone.csv'):
                print('Файл не создан')
            else:
                delete_file(input("Что удалить: "))
main()